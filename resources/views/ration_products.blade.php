...
<!-- Product Start -->
  ...
  <strong>Sorry!</strong> No Product Found.
    ...
    @foreach($products->chunk(4) as $items)
        ...
        <img src="{{ URL::asset('public/assets/images/products') }}/{{ $item->product_image }}" class="img-fluid">
      </div>
      <div class="col-6 col-lg-4 mb-3">
        <p><span class="font-weight-bold text-c1">Name: </span><span>{{ $item->product_name }}</span></p>
        <p><span class="font-weight-bold text-c1">Weight: </span><span>{{$item->product_weight}}</span></p>
        <p><span class="font-weight-bold text-c1">Rate: </span><span> {{$item->product_price}}/-</span></p>
        <!--p><span class="font-weight-bold text-c1">Mrp(Rs.): </span><span>{{$item->product_mrp}}/-</span></p>
        <p><span class="font-weight-bold text-c1">Save(Rs.): </span><span>{{$item->product_save_mrp}}/-</span></p-->
        <button id="{{$item->id}}" class="btn btn-danger text-sm add2Cart"><span class="glyphicon glyphicon-shopping-cart"></span> <i class="fa fa-plus" aria-hidden="true"></i>Add Cart</button>
      </div>
